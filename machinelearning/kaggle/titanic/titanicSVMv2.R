...
### READING DATA ###
train <- read.table("https://raw.githubusercontent.com/mepotts/datascience/master/machinelearning/kaggle/titanic/data/train.csv", sep = ",", header = TRUE)
test  <- read.table("https://raw.githubusercontent.com/mepotts/datascience/master/machinelearning/kaggle/titanic/data/test.csv", sep = ",", header = TRUE)
test$Survived <- 0
...
model1 <- train(Survived ~ Pclass + Sex + Age + SibSp + Parch + Embarked + Fare + Pclass:Sex + Pclass:Age, # Survived is a function of the variables we decided to include
...
write.table(test_new[, c("PassengerId", "Survived")], file = "TitanicSubmissionSVM2.csv", col.names = TRUE, row.names = FALSE, sep = ",")